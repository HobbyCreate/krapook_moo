...
import { useEditPocketName, useEditPocketLimit } from '@/customHooks/pocketHooks';
...
export default function EditPocketModal({ isOpen, onClose, pocket }: EditPocketModalProps) {
    const { data: balanceData } = useBalanceHook();
    const user = useAuthStore((state) => state.user);
    const loading = useAuthStore((state) => state.loading);
    
    const { mutate: editName, isPending: isNamePending } = useEditPocketName();
    const { mutate: editLimit, isPending: isLimitPending } = useEditPocketLimit();

    const [pocketName, setPocketName] = useState(pocket?.name || "");
    const [limitOnChange, setLimitOnChange] = useState<number>(pocket?.limit || 0);

    const [localBalance, setLocalBalance] = useState<number>(() => Number(balanceData?.amount ?? 0));


    const maxLimit = Number(balanceData?.amount ?? 0) + Number(pocket?.limit ?? pocket?.initialLimit ?? 0);

    const oldLimit = pocket?.limit ?? pocket?.initialLimit ?? 0;
    const originalBalance = Number(balanceData?.amount ?? 0);

    const onlimitHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
        const newLimit = Number(event.target.value);
        setLimitOnChange(newLimit);
        
        const diff = newLimit - oldLimit;
        setLocalBalance(originalBalance - diff);
    };

    const handlePercentageClick = (percentage: number) => {
        const calculated = Math.floor((maxLimit * percentage) / 100);
        setLimitOnChange(calculated);
        
        const diff = calculated - oldLimit;
        setLocalBalance(originalBalance - diff);
    };
    ...
    const handleUpdatePocketClick = () => {
        ...
        if (!user || !pocket) {
            alert("ไม่พบข้อมูลผู้ใช้หรือกระเป๋าที่ต้องการแก้ไข");
            return;
        }
        ...
        const checkIfFinished = () => {
            ...
            if (pendingRequests === 0) {
                if (!hasError) {
                    alert("แก้ไขกระเป๋าสำเร็จ!");
                    onClose();
                }
            }
        };
        
        const isNameChanged = pocketName.trim() !== "" && pocketName !== pocket.name;
        const isLimitChanged = Number(limitOnChange) !== pocket.limit;

        if (!isNameChanged && !isLimitChanged) {
            onClose();
            return;
        }
        ...
        if (isNameChanged) {
            pendingRequests++;
            editName(
                { id: pocket.id, name: pocketName },
                {
                    onError: (err) => {
                        console.error("Error updating name:", err);
                        hasError = true;
                        alert("เกิดข้อผิดพลาดในการแก้ไขชื่อกระเป๋า");
                    },
                    onSettled: checkIfFinished,
                }
            );
        }
        ...
        if (isLimitChanged) {
            ...
            editLimit(
                {
                    id: pocket.id,
                    newLimit: Number(limitOnChange),
                    userId: user.id,
                    oldLimit: pocket.initialLimit,
                },
                {
                    onError: (err) => {
                        console.error("Error updating limit:", err);
                        hasError = true;
                        alert("เกิดข้อผิดพลาดในการแก้ไขลิมิตกระเป๋า");
                    },
                    onSettled: checkIfFinished,
                }
            );
    ...
    const isPending = isNamePending || isLimitPending;
        ...
        >
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h2 className='text-xl font-bold text-gray-900'>แก้ไขข้อมูลกระเป๋า</h2>
                        <p className='text-xs text-gray-500'>ปรับเปลี่ยนชื่อหรือวงเงินงบประมาณของคุณ</p>
                    </div>
                </div>
                ...
                <div className='bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-4 sm:p-5 text-white mb-6 shadow-sm flex items-center justify-between'>
                    <div>
                        ...
                        <div className='text-xl sm:text-2xl font-extrabold mt-0.5'>
                            {Number(localBalance).toLocaleString()} <span className='text-sm font-semibold text-emerald-100!'>THB</span>
                        </div>
                    </div>
                </div>
                ...
                <div className='flex gap-3 mt-8'>
                    ...
                    <button 
                        type='button'
                        disabled={isPending}
                        onClick={handleUpdatePocketClick}
